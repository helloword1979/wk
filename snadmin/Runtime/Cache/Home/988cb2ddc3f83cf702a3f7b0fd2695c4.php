...
<body>
  ...
  <tr>
    <td> <form id="form1" name="form1" method="post" action="/admin.php/Home/Index/jiaoyi">
	 
   <input name="user" type="text" class="dfinput" id="user" />
	<input name="" type="submit" class="btn" value="确认搜索"/>
      </form></td>
	  <td>
	    <form id="form1" name="form1" method="post" action="/admin.php/Home/Index/jiaoyi">
  ...
  <tr>
    <td>总求购数量:<?php echo ($countlkb); ?> ||总价格:<?php echo ($countjb); ?>|| 订单数量：<?php echo ($count); ?> </td>
    <td align="right">&nbsp;</td>
  </tr>
</table>
     ...
     <table class="tablelist">
		...
		<td>
		<?php if($v["g_level"] == 0): ?>Vip0<?php endif; ?>
		  <?php if($v["g_level"] == 1): ?>Vip1<?php endif; ?>
		   <?php if($v["g_level"] == 2): ?>Vip2<?php endif; ?>
		    <?php if($v["g_level"] == 3): ?>Vip3<?php endif; ?>
			 <?php if($v["g_level"] == 4): ?>Vip4<?php endif; ?>
			 <?php if($v["g_level"] == 5): ?>Vip5<?php endif; ?>
			  <?php if($v["g_level"] == 6): ?>Vip6<?php endif; ?>
			   <?php if($v["g_level"] == 7): ?>Vip7<?php endif; ?>
		</td>
		<td><?php echo ($v["jydate"]); ?></td>
		<td><a href="/uploads/<?php echo ($v["imagepath"]); ?>" target="_blank"><img src="/uploads/<?php echo ($v["imagepath"]); ?>"  style="width:40px;"/></a></td>
		<td><a href="/admin.php/Home/Index/qxjy/id/<?php echo ($v["id"]); ?>" class="tablelink">删除</a></td>
