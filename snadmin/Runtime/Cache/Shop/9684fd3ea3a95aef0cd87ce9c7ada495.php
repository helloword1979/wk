...
<body>
    ...
    <div><?php $toggleshop = M('system')->where(['SYS_ID'=>'1'])->getField('toggleshop');?>
      <form id="form1" name="form1" method="post" action="/admin.php/Shop/Index/toggleShop">
    <ul class="forminfo">
	 ...
	 <table class="tablelist">
	   ...
	   <script>
			function edit(ob){
				...
				$.post("/admin.php/Shop/Index/edit",{name:name,id:id},function(data){
					if(data){
						alert("修改成功");
					}else{
						alert("修改成功");
			...
			function del(ob){
				var obj = $(ob);
				var id = obj.parent().parent().children().eq(0).html();
				$.post("/admin.php/Shop/Index/del",{id:id},function(data){
					if(data){
						alert("删除成功");
